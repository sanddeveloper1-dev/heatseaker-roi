 ...
 * 
 * Extracts totals from all date-named TEE sheets (generated by 05-ROIProcessing.js)
 * and marks them as processed. Appending to TOTALS is now handled by 07-DailyTotalsSync.js
 * 
 * Features:
 * - Loops through all sheets named as dates (MM/dd/yy format)
 * - Extracts BET, COLLECT, BETS, and WINS from specified cells
 * - Marks sheets as processed (BR1 = TRUE) after extraction
...
/**
 * Main entry point - extracts totals from all date-named TEE sheets and marks as processed
 * Note: Appending to TOTALS is now handled by syncDailyTotals() in 07-DailyTotalsSync.js
 * @returns {Object} Summary of processing
 */
function extractRoiTotals() {
	const startTime = Date.now();
	console.log('🚀 Starting ROI Totals extraction...');
	...
	try {
		...
		for (let i = 0; i < dateSheets.length; i++) {
			...
			if (isSheetProcessed_(sheet)) {
				...
				console.log(`⏭️ Skipping ${sheetName} - already processed (BR1 is TRUE)`);
				continue;
			}
			...
			try {
				...
				if (totals) {
					...
					console.log(`✅ Processed ${sheetName}: BET=${totals.bet}, COLLECT=${totals.collect}, BETS=${totals.bets}, WINS=${totals.wins} (marked BR1 as TRUE)`);
				} else {
					summary.skippedSheets++;
					console.log(`⚠️ Could not extract totals from ${sheetName} - missing or invalid data`);
				}
			} catch (error) {
		...
		console.error('❌ Fatal error in extractRoiTotals:', error);
